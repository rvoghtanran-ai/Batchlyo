...
async function startServer() {
        ...
        try {
            ...
            for (const batch of allResults) {
                for (const kw of batch) {
                    ...
                    if (clean === seed.toLowerCase() || clean.length < 4)
                        continue;
                    if (!seen.has(clean)) {
                        seen.add(clean);
                        keywords.push(clean);
    ...
    app.get("/api/pinterest/profile", async (req, res) => {
        try {
            const username = (req.query.username || '').trim().replace(/^@/, '').replace(/\/$/, '');
            if (!username)
            ...
            if (!resp.ok)
                return res.status(resp.status).json({ error: `Pinterest returned ${resp.status}` });
            ...
            for (const match of pinMatches) {
                try {
                    ...
                    const findPins = (obj, depth = 0) => {
                        if (depth > 8 || !obj || typeof obj !== 'object' || pins.length >= 50)
                            return;
                        ...
                        if (Array.isArray(obj)) {
                            for (let i = 0; i < Math.min(obj.length, 60); i++)
                                findPins(obj[i], depth + 1);
                        }
                        else {
                            for (const key of Object.keys(obj))
                                findPins(obj[key], depth + 1);
                        }
                    };
                    findPins(data);
                }
            ...
            for (const pin of pins) {
                if (pin.boardName)
                    boardDist[pin.boardName] = (boardDist[pin.boardName] || 0) + 1;
            }
            ...
            const wordFreq = {};
            for (const pin of pins) {
                const words = (pin.title + ' ' + pin.description).toLowerCase().replace(/[^a-z\s]/g, '').split(/\s+/).filter((w) => w.length > 3 && !stopWords.has(w));
    ...
    async function resolveGeo(ip) {
        ...
        try {
            ...
            const flag = data.countryCode
                ? data.countryCode.toUpperCase().replace(/./g, (c) => String.fromCodePoint(c.charCodeAt(0) + 127397))
                : '??';
    ...
    app.get('/api/visitors/live', (req, res) => {
        ...
        const IDLE_THRESHOLD = 30_000;   // idle if no heartbeat for 30s

        const active = [];
        for (const [id, session] of visitorSessions.entries()) {
            if (now - session.lastSeen > ACTIVE_THRESHOLD) {
                visitorSessions.delete(id); // Prune stale sessions
                continue;
            }
        ...
        // Sort: active first, then by session duration desc
        active.sort((a, b) => {
        ...
        const avgDuration = active.length > 0
            ? Math.floor(active.reduce((sum, s) => sum + s.durationMs, 0) / active.length)
            : 0;
        const pageFreq = {};
